...
const ProjectGridSection = ({
  ...
  return (
    <section
      className={cn(
        "box-border flex flex-col justify-center items-center p-0 gap-2.25 w-full bg-white border-x border-[rgba(114,123,142,0.1)] py-10",
        className
      )}
    >
      {rows.map((row, rowIndex) => (
        <div
          key={rowIndex}
          className="flex flex-row items-center p-0 gap-2.25 w-284 max-w-full"
        >
          {row.map((project) => (
