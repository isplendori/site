...
const MethodStepsSection = ({
  ...
  return (
    <section
      className={cn(
        "box-border flex flex-row items-start w-full bg-white border-x border-[rgba(114,123,142,0.1)] px-13.5 py-11 gap-27",
        className
      )}
    >
      <div className="flex flex-col items-start p-0 gap-3.5 w-107.25 sticky top-25 self-start">
        <div className="flex flex-col items-start p-0 w-79.5">
          {badge && (
      ...
      <div className="flex flex-col items-stretch w-142.75">
            ...
            <div key={index} className="flex flex-row items-start gap-5 w-full">
              <div className="relative shrink-0 w-5 self-stretch">
                <div className="relative z-10 flex flex-row justify-center items-center w-5 h-5 bg-[#9E372A] rounded-full border border-[#E4E7EC]">
                  <span className="font-sans font-medium text-[12px] leading-3 text-white">
                    {index + 1}
                  </span>
                </div>
                {!isLast && (
